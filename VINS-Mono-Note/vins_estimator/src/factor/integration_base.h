...
class IntegrationBase
{
  public:
    ...
    //_acc_0：初始时刻加速度值  _gyr_0：初始时刻陀螺仪值  _linearized_ba：加速度零偏   _linearized_bg：陀螺仪零偏
    //jacobian:预积分量对自身的雅可比矩阵 初始值：在所有IMU数据来临之前，只有自身与自身有联系(导数为1)，α，β，γ相互之间以及他们与零偏之间没有任何联系
    //delta_p：α   delta_q：γ(一开始没有旋转，单位四元数)   delta_v：β
    ...
    {
        //噪声的协方差矩阵
        //噪声nk为18x1的向量：k时刻三维加速度计噪声、k时刻三维陀螺仪噪声、k+1时刻加速度计噪声、k+1时刻陀螺仪噪声、加速度计和陀螺仪零偏的bias随机游走
        //这里noise为公式推导中的V，是nk的协方差矩阵
        //这里赋值的意义是，认为k时刻和k+1时刻加速度噪声是相同的(属于他们的那个块赋值成了单位阵)；同理认为k时刻和k+1时刻的陀螺仪噪声相同；k时刻和k+1时刻的加速计或者陀螺仪为高斯白噪声  
    ...
    {
        // 相关时间差和传感器数据保留，方便后续repropagate(重新传播，初始化会用到)
        dt_buf.push_back(dt);
    ...
    /*中值积分 
    * Step 1 来一帧IMU数据，根据中值积分更新预积分量
    * Step 2 误差传递，计算F和V矩阵
    * Step 3 F矩阵更新雅可比矩阵，F和V更新预积分量的置信度(协方差矩阵)，因为优化时不仅需要观测值还需要观测的置信度
    * //!方程的推导用到了误差卡尔曼(基于的假设是连续两IMU帧间变化非常小，可以用旋转向量表示旋转？(旋转向量具有周期性，但小旋转时不存在这个问题？))
    * @param[in] _dt      时间
    * @param[in] acc_0   k时刻加速度值
    * @param[in] gyr_0   k时刻陀螺仪值
    * @param[in] _acc_1  k+1时刻陀螺仪值
    * @param[in] _gyr_1  k+1时刻陀螺仪值
    * @param[in] delta_p delta_q, delta_v k时刻的预积分量 α、γ、β
    * @param[in] linearized_ba, linearized_bg 零偏 根据建模，两IMU帧之间零偏是不变的
    * @param[out] result_delta_p, result_delta_q, result_delta_v  得到的新的预积分量 α、γ、β？
    * @param[out] result_linearized_ba, result_linearized_bg：零偏是直接赋值的没有变   update_jacobian表示是否更新雅可比，调用时始终为1
    */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        // Step 1 首先中值积分更新状态量
        //!注意：预积分计算的是k时刻到k+1时刻的，这里k是图像帧中，再来一帧图像表示第k帧，再来一帧图像是k+1帧
        //!预积分量α、γ、β要转移到k时刻坐标系下

        //!因此加速度计的处理：本身加速度计的值是IMU传感器body系下，通过一个旋转矩阵R从IMU坐标下转到k时刻坐标系下再运算，这里R就是γ？
        //!(这里一部分可以参考深蓝学院的公式推导)
        //_acc_0第i帧(imu帧)的加速度计的值，经过旋转转到k帧下un_acc_0  
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);
        //!陀螺仪：负责计算帧间的旋转，因此不需要转换，直接得到中值(陀螺仪帧的i时刻和i+1时刻各自减去零偏后相加再除以2)
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
        //?首先更新旋转量，其它的和他有关系
        //第i+1帧(IMU帧)旋转的预积分量  (看笔记)  另外按道理这里应该要做个归一化，由于_dt非常小，可以近似认为是个单位四元数
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);
        //_acc_1 第i+1帧(imu帧)的加速度计的值，通过刚计算出的i+1帧旋转，转到k+1帧坐标系下的加速度计值
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        //加速度计中值(这里是bk系下，这段时间的加速度值)
        ...
        if(update_jacobian)
        {
            ...
            //cout<<"A"<<endl<<A<<endl;
            //白噪声是个零均值的高斯分布，取反是不影响的；下面代码有些正负号可能会与论文或公式推导中有区别(但本质上没有影响)
    ...
    * 
    * @param[in] _dt      时间
    * @param[in] _acc_1   加速度数据
    * @param[in] _gyr_1   陀螺仪数据
    */
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
         ...
         * @param[in] linearized_ba, linearized_bg 零偏 根据建模，两IMU帧之间零偏是不变的
         * @param[out] result_delta_p, result_delta_q, result_delta_v  得到的新的预积分量 α、γ、β？
         * @param[out] result_linearized_ba, result_linearized_bg：零偏是直接赋值的没有变   1表示更新雅可比，始终为1
         */
