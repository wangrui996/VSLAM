        ...
        mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
    ...
    //        |0   0    1  |
    //构造相机内参矩阵
    cv::Mat K = cv::Mat::eye(3,3,CV_32F);  //3行3列的单位矩阵
    K.at<float>(0,0) = fx;
    K.at<float>(1,1) = fy;
    K.at<float>(0,2) = cx;
    K.at<float>(1,2) = cy;
    ...
    // 图像矫正系数
    // [k1 k2 p1 p2 k3]
    cv::Mat DistCoef(4,1,CV_32F);  //畸变参数矩阵 4x1
    ...
    //插入关键帧或者检查重定位时的需要根据判断时间间隔，最小就是0，最大肯定和帧率有关
    mMinFrames = 0;
    mMaxFrames = fps;
    ...
    //双目初始化时，会双目做立体匹配生成初始的3D点
    if(sensor==System::STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);

    // 在单目初始化的时候，会用mpIniORBextractor来作为特征点提取器，提2倍的特征点(初始化时)
