...
* [单目初始化](#单目初始化)
    * [特征点坐标归一化目的](#特征点坐标归一化目的)  
    * 
* [ORB_SLAM2中的卡方检验](#ORB_SLAM2中的卡方检验)
...
#### 特征点坐标归一化
   目的：归一化在数据预处理中非常常见，主要是解决特征间单位(尺度)可能不同或者相差过大的问题。归一化相当于对原数据做线性变换，使得不同单位或者不同量级的数据能够放在一起比较或者加权。SLAM问题中，像素点坐标通常为0到几百，根据[基础矩阵和单应矩阵]()最后求解的形式可以看出，系数矩阵A中，有u，v以及他们的组合乘积的形式，本身像素点的齐次坐标中各个分量的数量级就可能有差距，在系数矩阵中各分量间就更明显了。有大佬就发现直接用八点法求解AF = 0或者AH = 0的时候，效果并不好，就提了一种改进的八点法，就是先归一化再求解。
   方法：对特征点坐标先平移再缩放，使特征点质心移动到原点，且一阶绝对矩为1
   首先补充一个知识，一阶矩：随机变量的期望；  二阶矩:随机变量平方的期望； 一阶绝对矩：随机变量与均值绝对值的平均
   具体步骤：
   
   
### ORB_SLAM2中的卡方检验
[SLAM中的卡方分布](https://www.cnblogs.com/hardjet/p/11708202.html)  
